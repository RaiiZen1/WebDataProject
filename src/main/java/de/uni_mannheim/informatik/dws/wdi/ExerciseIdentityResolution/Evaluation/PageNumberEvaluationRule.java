...
public class PageNumberEvaluationRule extends EvaluationRule<Book_DF, Attribute>{
    @Override
	public boolean isEqual(Book_DF record1, Book_DF record2, Attribute schemaElement) {
		if(record1.getPageNumber()==-1 && record2.getPageNumber()==-1)
			return true;
		else if(record1.getPageNumber()==-1 ^ record2.getPageNumber()==-1)
			return false;
		else
			return record1.getPageNumber() == record2.getPageNumber();
	}
